...
with col1:
    ...
    insights = []
    if no_show_rate > 25:
        insights.append(f"• **HIGH NO-SHOW RATE**: {no_show_rate:.1f}% (Industry standard: 10-15%)")
    
    if len(rep_performance) > 1:
        best_rep = rep_performance.iloc[0]
        worst_rep = rep_performance.iloc[-1]
        if best_rep['conversion_rate'] - worst_rep['conversion_rate'] > 20:
            insights.append(f"• **PERFORMANCE GAP**: {best_rep['rep']} ({best_rep['conversion_rate']:.1f}%) vs {worst_rep['rep']} ({worst_rep['conversion_rate']:.1f}%)")
    
    if len(segment_performance) > 0:
        worst_segment = segment_performance.loc[segment_performance['conversion_rate'].idxmin()]
        if worst_segment['conversion_rate'] < 20:
            insights.append(f"• **UNDERPERFORMING SEGMENT**: {worst_segment['segment']} at {worst_segment['conversion_rate']:.1f}% conversion")
    
    if not pd.isna(avg_lead_time) and avg_lead_time > 14:
        insights.append(f"• **LONG LEAD TIMES**: {avg_lead_time:.1f} days average (may impact urgency)")
    
    if not insights:
        insights.append("• No critical issues detected with current filters")
